...
This version removes all rendering of notes ([n], <sup>, <span>) from the
Markdown output, while still parsing the structure and keeping all numbering
logic for paragraphs and lists exactly as before.

Modifications (2025-08-04) :
  * ``extract_article_number`` a été améliorée pour prendre en charge les suffixes
    ordinaux reconnus (bis, ter, quater, etc.) lorsqu’ils figurent à côté du
    numéro principal. Les lettres séparées par un espace des chiffres sont
    considérées comme faisant partie du numéro (par exemple, « 40 d » devient « 40d »).
  * ``unify_link_suffixes`` est une nouvelle méthode qui fusionne les lettres et
    suffixes qui suivent directement un lien : « **[[ Art. 329 CO]]** g bis »
    devient « **[[Art. 329g bis CO]]** ».
  * ``format_article_markdown`` invoque ``unify_link_suffixes`` sur chaque ligne
    avant de l’ajouter au Markdown, garantissant que la conversion complète et
    les fichiers découpés conservent la même présentation.
...
class SwissCodeConverter:
    ...
    def __init__(self, config: Optional[Dict[str, Union[str, int]]] = None) -> None:
        self.config: Dict[str, Union[str, int]] = config.copy() if config else CONFIG.copy()
        self.ns = {'akn': AKN_NS, 'fedlex': FEDLEX_NS}
        self.in_final_section: bool = False
        self.suffix_counter: int = int(self.config.get('suffix_counter', 1))
    ...
    def extract_article_number(self, article_element: ET.Element) -> str:
        ...
        if num_element is not None:
            num_text = ' '.join(''.join(num_element.itertext()).split())
            suffixes = r"bis|ter|quater|quinquies|sexies|septies|octies|nonies|decies"
            pattern = re.compile(rf'(\d+)\s*([a-z])?\s*(?:({suffixes}))?', re.IGNORECASE)
            m = pattern.search(num_text)
            if m:
                digits = m.group(1)
                letter = m.group(2) or ''
                suffix = m.group(3) or ''
                base = f"{digits}{letter}"
                return f"{base} {suffix}".strip()
        return ""

    def extract_marginal_notes(self, article_element: ET.Element) -> str:
        hierarchy: List[str] = []
        current: Optional[ET.Element] = article_element
    ...
    # replace_article_references has been fully removed!

    def unify_link_suffixes(self, text: str) -> str:
        # (Facultatif: tu peux aussi supprimer cette fonction si tu ne veux plus du tout de liens automatiques)
        return text
    ...
    def format_article_markdown(self, article_number: str, marginal_notes: str,
        ...
        for para_num, para_text, _notes, level in paragraphs:
            indent = "   " * level
            # ICI : suppression de replace_article_references, on garde le texte brut
            converted_text = para_text
            if para_num:
                colored_num = f"<span style=\"color:yellow\"><small>{para_num}</small></span>"
                line = f"{indent}**{colored_num}** {converted_text}"
            else:
                line = f"{indent}{converted_text}"
            unified_line = self.unify_link_suffixes(line)
            markdown_lines.append(unified_line)
        markdown_lines.append("")
        return "\n".join(markdown_lines)
    ...
    def update_counter_after_save(self, article_num: str) -> None:
        if "bis" in article_num:
...
if __name__ == "__main__":
    if tk is not None:
        ...
        # When Tkinter is not available, no GUI is started.
        pass
